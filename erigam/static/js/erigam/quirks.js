define("erigam/quirks", [], function() {
	...
	return {
		apply: function(text, character) {
			...
			for (var i=0; i < character.replacements.length; i++) {
				var re, str1, str2;
				var replacement = character.replacements[i];
				if (replacement[1] == " ") {
					replacement[1] = "";
				}
				...
				if (replacement[0].match(/\/.*?\//)) {
					...
					try {
					   re = new RegExp(str1, "g"); 
						...
						console.log("A young person stands in their bedroom. They don't know Regexp.");
					}
					...
					str1 = replacement[0];
					str2 = replacement[1];
					re = new RegExp(RegExp.quote(str1), "g"); 
					text = text.replace(re, str2);
				}
			}
			...
			// Prefix
			if (character.quirk_prefix !== '' || character.quirk_suffix !== '') {
				text = character.quirk_prefix+' '+text+' '+character.quirk_suffix;
			}
